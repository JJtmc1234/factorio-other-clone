...
function distFromSpawn(tx, ty) {
    return Math.sqrt(tx * tx + ty * ty);
}
export function generateResources(tiles, cx, cy) {
    ...
    for (const cfg of ORE_CONFIGS) {
        if (rng.next() > cfg.baseFrequency)
            continue;
        if (distCenter < SPAWN_CLEAR_RADIUS * 2)
            continue;
        ...
        for (let ty = 0; ty < CHUNK_SIZE; ty++) {
            for (let tx = 0; tx < CHUNK_SIZE; tx++) {
                ...
                if (distFromSpawn(worldX, worldY) < SPAWN_CLEAR_RADIUS)
                    continue;
                if (tiles[ty][tx].type === "water")
                    continue;
                if (tiles[ty][tx].resource)
                    continue;
